...
for i = 1:length(Pfasim2)
    ...
    Pdsim(i) = Det / (N);
    yshadow = xsend.*Shadow + noise;
    yCPshadow = reshape(yshadow, Ts, []);
    yshadow = yCPshadow(Tc+1:end, :);
    Yshadow = fft(yshadow, Td);
    Yshadow = Yshadow(:);
    DetShadow = sum(abs(Yshadow) > thresh(i));
    Pdsim2(i) = DetShadow / (N);
end
figure;
plot(Pfasim2,Pdsim)
hold on
...
title(" ROC Plot for $ SNR = -10dB$ with and without shadowing ",'interpreter','latex');
legend('AWGN','Shadowing')
grid on;